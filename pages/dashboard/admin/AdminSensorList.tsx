...
const AdminSensorList: React.FC<SensorListProps> = ({ sensors }) => {
  ...
  const validRecords = selectedSensor?.temperatureRecords?.filter(
    ...
    ...r,
    currentTempC: Number(r.currentTempC), // convert to number
  })) || [];

  const chartData = validRecords.map(r => Number(r.currentTempC.toFixed(2)));
  ...
  const minTemp = chartData.length > 0 ? Math.floor(Math.min(...chartData)) - 1 : 0;
  const maxTemp = chartData.length > 0 ? Math.ceil(Math.max(...chartData)) + 1 : 10;
    ...
    <>
                  ...
                  navigate(
                    `/admin/sensors/sensor/${selectedSensor._id}`,
                    {
                      state: {
                        ...
                        storeId: selectedSensor.storeId,     
          subStoreId: selectedSensor.subStoreId
                      },
                    }
                  );
                }}
      ...
      {selectedSensor && (
        <div className="mt-6 p-6 bg-gray-50 dark:bg-gray-900 rounded-2xl shadow-lg">
          <h1 className="text-2xl font-bold mb-6 text-gray-800 uppercase dark:text-gray-100 flex items-center gap-3">
