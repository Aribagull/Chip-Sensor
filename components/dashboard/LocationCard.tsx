...
import { MapPin, Box, Thermometer, Edit, Trash2, ChevronRight, Store, Layers, AlertTriangle } from 'lucide-react';
...
const LocationCard: React.FC<LocationCardProps> = ({
  ...
  const totalSubStoreAlerts = location.subStores.reduce(
  (acc, sub) =>
    acc +
    (sub.sensors?.filter(
      (sensor) =>
        sensor.status === "on" &&
        sensor.notificationStatus === "on"
    ).length || 0),
  0
);
...
">
        ...
        <div className="p-6 flex justify-between items-start bg-gray-50 dark:bg-slate-800 rounded-t-2xl">
          <Link
            ...
            state={{ store: location , storeName: location.storeName }}
...
<div
  className={`
    flex items-center gap-2 px-3 py-1 rounded-full
    text-sm font-medium
    ${location.totalSubStoreAlerts > 0
...
>
  ...
  {location.totalSubStoreAlerts} Alerts
</div>
      ...
      "
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`h-3 w-3 rounded-full ${activeSensors > 0 ? "bg-green-400" : "bg-gray-400"
                        }`}
                    />
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-slate-400">
                    <Thermometer
                      className={`h-4 w-4 ${activeSensors > 0 ? "text-green-500" : "text-gray-400"
                        }`}
                    />
